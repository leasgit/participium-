...
class CategoryService:
    ...
    def update_category(self, category_id: int, *, name: str | None = None, is_active: bool | None = None):
        category = self.get_category(category_id) #existing_category
        if name:
            cleaned_name = name.strip() # new name
            duplicate = self.category_repository.get_by_name(cleaned_name) #check input name whether exists
            if duplicate and duplicate.id != category.id:
                raise ValidationError("Category name already exists.")
            category.name = cleaned_name # assign new name to old name
